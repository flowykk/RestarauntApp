...
- Упрощение структуры кода. Ведь код, связанный с каждым отдельным состоянием заказа, никак не зависит от самого заказа и находится в классах самих состояний. + данный паттерн позволяет избегать условия `if` при работе с разными состояними объекта.
